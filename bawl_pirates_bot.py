...
async def callback_inline(call):
    ...
    if call.data == "subscriber":
        ...
        get_signal_button = types.InlineKeyboardButton(text="🔑 Hackear o jogo 🔑", callback_data="get_signal")
    ...
    if call.data == "instruction":
        ...
        await bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.id)
        await bot.send_photo(chat_id=call.message.chat.id,photo=open('resources/instruction.jpg', 'rb') , caption="O bot é baseado e treinado na rede neural 🖥 CLAUD-3.\n\n"
                                                                  ...
                                                                  "🟡 3. Vá para a seção de jogos 1win e selecione o jogo 💀'Brawl Pirates'.\n\n"
                                                                  "🟡 4. Copie o Hash ID do jogo. É importante!\n\n"
                                                                  "🟡 5. Envie Hash ID para o bot e selecione a caveira com base em um sinal do bot.\n\n"
                                                                  "🟡 6. Se o sinal não tiver sucesso, recomendamos duplicar (X²) a aposta para cobrir completamente a perda no próximo sinal.",
                         reply_markup=keyboard,
                         parse_mode="HTML"
                             )
    ...
    if call.data == "get_signal":
        async with session_maker() as session:
            ...
            if (user1.scalar_one_or_none() is None) or (user2.scalar_one_or_none().casino_id is None):
                ...
                await bot.send_message(chat_id=call.message.chat.id, text='Envie o HASH ID do seu jogo:')
...
async def got_hash(message):
    if len(message.text) == 128:
        async with session_maker() as session:
            ...
            if (user1.scalar_one_or_none() is None) or (user2.scalar_one_or_none().casino_id is None):
                ...
                await bot.send_message(chat_id=message.from_user.id,
                                       text="Por favor, registre-se antes de receber um sinal", reply_markup=keyboard)
            else:
                hashik = message.text
                await session.execute(update(User).where(User.telegram_id == message.from_user.id).values(game_hash=hashik))
                await session.commit()
                vid = await bot.send_video(chat_id=message.from_user.id,video=open('resources/game.MP4', 'rb'), caption="🔘 Analisando seu jogo ▪️▪️▪️")
        ...
        await bot.send_message(chat_id=message.from_user.id, text='Hash inválido!\n'
                                                                  'Por favor, envie um Hash ID correto.', reply_to_message_id=message.message_id)
